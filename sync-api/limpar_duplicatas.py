...
def limpar_duplicatas_formulas():
    ...
    for i in range(0, len(duplicatas_ids), 100):
        ...
        if resp_delete.status_code == 204:
            ...
            print(f"   [OK] Lote {i//100 + 1}: {len(lote)} duplicatas deletadas (total: {total_deletado})")
        else:
            print(f"   [ERRO] Erro ao deletar lote {i//100 + 1}: {resp_delete.status_code}")

    print(f"\n[SUCESSO] CONCLUIDO - {total_deletado} duplicatas deletadas!")
    print("="*80 + "\n")
    return True
...
def limpar_duplicatas_itens():
    ...
    for i in range(0, len(duplicatas_ids), 100):
        ...
        if resp_delete.status_code == 204:
            total_deletado += len(lote)
            if (i + 100) % 1000 == 0:
                print(f"   [OK] Progresso: {total_deletado} duplicatas deletadas")
        else:
            print(f"   [ERRO] Erro ao deletar lote {i//100 + 1}: {resp_delete.status_code}")
...
if __name__ == "__main__":
    ...
    if sucesso_formulas and sucesso_itens:
        ...
        print("\n" + "="*80)
        print("[ERRO] LIMPEZA CONCLUIDA COM ERROS")
        print("="*80 + "\n")
